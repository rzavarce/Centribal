...
<script>
  ...
  export default defineComponent({
    ...
    mounted () {
      ...
      let price = []
      let tax = []
      httpClient.get('orders/edit/' + pk + '/', {}, false, false, false).then((response) => {
        response.order_data[0].order_detail.forEach(function(product) {
          ...
          const founded = response.products_list.find(e => e.value === product.id)
          price.push(founded.price)
          tax.push(founded.tax)
